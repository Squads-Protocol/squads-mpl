...
describe("Multisig and Programs", () => {
    ...
    describe('Basic functionality', () => {
        ...
        it(`Create Multisig - MS: ${msPDA.toBase58()}`, async () => {
            ...
            await program.methods.create(threshold, randomCreateKey, memberList.map(m=>m.publicKey).concat(creator.publicKey))
                .accounts({
                    multisig: msPDA,
    ...
    describe('Program upgrades', () => {
        ...
        it(`Create upgrade with buffer and deploy it - MS ${msPDA.toBase58()}`, async function(){
            ...
            const [ixPDA] = await getIxPDA(txPDA, newIxIndexBN, program.programId);
            // create the instruction pda
            const [ix2PDA] = await getIxPDA(txPDA, newIx2IndexBN, program.programId);
            ...
            const addUpgradeTx = await program.methods.addInstruction(upgradeIx)
                .accounts({
                    multisig: msPDA,
                    transaction: txPDA,
                    instruction: ixPDA,
            ...
            const [authorityPDA] = await getAuthorityPDA(msPDA, new anchor.BN(1,10), program.programId);
            ...
            const addUpgradeFollowupIx = await program.methods.addInstruction(updateUpgradeIx)
                .accounts({
                    multisig: msPDA,
                    transaction: txPDA,
                    instruction: ix2PDA,
            ...
            try {
                await programProvider.sendAndConfirm(addUpgradeTx);
            } catch (e){
                console.log(e);
            }
            ...
            for (let i=0; i < memberList.length; i++){
                ...
                const approvalState = await program.account.msTransaction.fetch(txPDA);
                ...
                try {
                    await programProvider.connection.requestAirdrop(memberList[i].publicKey, anchor.web3.LAMPORTS_PER_SOL);
                    const approveTx = await program.methods.approveTransaction()
                        .accounts({
                            multisig: msPDA,
                            transaction: txPDA,
                        ...
                        .transaction();
                    try {
                        await programProvider.sendAndConfirm(approveTx, [memberList[i]]);
                        return true;
                    }catch(e){
